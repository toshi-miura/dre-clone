...
public class PackageSelectPage extends DREPage {
	...
	FileEntryMeasurerPackage getSelectedPackage() {
	...
	void createContents(Composite parent) {
		List<FileEntryMeasurerPackage> packageList = new ArrayList<FileEntryMeasurerPackage>();
		for (File packageFile : PACKAGE_DIR.listFiles()) {
			try {
				packageList.add(FileEntryMeasurerPackage.importPackage(frame, packageFile));
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		packages = packageList.toArray(new FileEntryMeasurerPackage[packageList.size()]);
		...
		for (FileEntryMeasurerPackage e : packages) {
			String className = e.getKey();
			for (OptionEntry option : e.getOptionMap().values()) {
	...
	void setActivePackage(FileEntryMeasurerPackage pack) {
	...
	void hiddened() {
		if (getSelectedPackage() != null) {
			FileEntryMeasurerPackage selectedPackage = getSelectedPackage();
